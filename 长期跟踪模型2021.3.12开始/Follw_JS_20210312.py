...
def insertDB(content):
    ...
    try:
        ...
        cursor.executemany('insert into F_js20210312_M ({0}) values ({1})'.format(f_lcode, f_ls[:-1]), content)
        connection.commit()
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
#create table F_js20210312_M(id int not null primary key auto_increment, J1720 FLOAT, J1803 FLOAT,J3612 FLOAT, J4555 FLOAT, LastTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP ) engine=InnoDB  charset=utf8;
