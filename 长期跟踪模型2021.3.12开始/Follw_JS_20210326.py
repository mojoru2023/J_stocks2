...
def insertDB(content):
    ...
    try:
        ...
        cursor.executemany('insert into F_js20210326_M ({0}) values ({1})'.format(f_lcode, f_ls[:-1]), content)
        connection.commit()
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
#create table F_js20210326_M(id int not null primary key auto_increment, J3612 FLOAT,  J4680 FLOAT,  J4681 FLOAT,  J4928 FLOAT,  J7013 FLOAT, LastTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP ) engine=InnoDB  charset=utf8;
