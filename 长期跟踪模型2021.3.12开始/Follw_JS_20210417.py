...
def insertDB(content):
    ...
    try:
        ...
        cursor.executemany('insert into F_js20210417_M ({0}) values ({1})'.format(f_lcode, f_ls[:-1]), content)
        connection.commit()
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
#create table F_js20210417_M(id int not null primary key auto_increment,  J5105 FLOAT,  J5108 FLOAT,  J5406 FLOAT, J1605 FLOAT,  LastTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP ) engine=InnoDB  charset=utf8;
